...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var counter_search = 0, counter_stats = 0;
            $.each(data.response.series, function(key, value) {
                ...
                if (dataType == "twitter-basic-stats") {
                    twitter_basic_stats_collector[counter_stats] = value.jsonData;
                    counter_stats++;
                }
            });
            ...
            if (twitter_basic_stats_collector.length > 0) {
                $("#data_container").append('<h4>Twitter-basic-stats</h4>');
                ...
                for (i in twitter_basic_stats_collector) {
                    ...
                    stats_container.append('<p class="no_padding">Tweet locations: <strong>' + value.tweet_locations + '</strong></p>');
                    stats_container.append('<p class="no_padding">User profile locations: <strong>' + value.user_locations + '</strong></p>');
                    stats_container.append('<p class="no_padding">Media shared: <strong>' + value.media_shared + '</strong></p>');
